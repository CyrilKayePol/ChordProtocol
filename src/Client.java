...
public class Client extends Thread{
	...
	public Client(String hostIpArgs, String hostPortArgs, String clientPortArgs) {

		initialize(hostIpArgs, hostPortArgs, clientPortArgs);
		joinRing();
		start();
		waitForMessage();
	}
	
	private void initialize(String hostIpArgs, String hostPortArgs, String clientPortArgs) {
		univIndex = 0;
		localIndex = 0;
		fingerNode = null;
		...
		hostPort = Integer.parseInt(hostPortArgs);
		hostIP = hostIpArgs;
		myPort = Integer.parseInt(clientPortArgs);
		try {
		...
		//myNode.setID(new BigInteger("56"));
	...
	public static void main(String[] args) {
        if (args.length != 3) {
            System.err.println(
                "Usage: java Client <host name> <host port number> <client port number>");
            System.exit(1);
        }
