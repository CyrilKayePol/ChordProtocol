...
public class Host extends Thread{
	...
	public Host(String ipArgs, String portArgs) {
		initialize(ipArgs, portArgs);
		createRing();
		populateFingerTable();
		start();
		waitForMessage();
	...
	private void initialize(String ipArgs, String portArgs) {
		val = 0;
		univIndex = 0;
		localIndex = 0;
		fingerNode = null;
		...
		myPort = Integer.parseInt(portArgs);	
		try {
			myIP = ipArgs;
		...
		pro = new Protocol(hostSocket, myIP, myPort);
			...
			receive();
				if(input instanceof Message) {
					...
					if(s[0].equals("asking for node of id")) {
						...
						n = m.getNode();
						//System.out.println("receiver find successor = "+n.getIP());
						...
						n = m.getNode();
						String name = s[1];
						//System.out.println("file neym download= "+name);
						sendFile = new SendFile("files/"+name, "files/");
						event = sendFile.getFileEvent();
	...
	public static void main(String[] args) {
        ...
        if (args.length != 2) {
            System.err.println(
                "Usage: java Host <host name> <port number>");
            System.exit(1);
        }
